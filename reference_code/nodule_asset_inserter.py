...
def crop_sitk_image(img: sitk.Image, slices_zyx: Tuple[slice, slice, slice]) -> sitk.Image:
    ...
    index = [xsl.start, ysl.start, zsl.start]
    size = [xsl.stop - xsl.start, ysl.stop - ysl.start, zsl.stop - zsl.start]
...
def create_asset(args: argparse.Namespace) -> None:
    ...
    mask_arr = (sitk.GetArrayFromImage(source_mask_img) == int(args.label)).astype(np.uint8)
    ...
    mask_crop = (sitk.GetArrayFromImage(mask_crop_img) == int(args.label)).astype(np.uint8)
    ...
    residual = (patch_arr - background_hu) * alpha
    residual = residual.astype(np.float32)

    centroid_lps, centroid_idx_xyz = centroid_physical_from_mask(mask_crop_img, label=int(args.label))
    ...
    sitk.WriteImage(mask_crop_img, str(out_dir / "mask_labelmap.nrrd"))
    ...
    metadata = {
        ...
        "notes": "Residual signal is patch_ct - estimated local donor background, multiplied by alpha.",
    }
...
def insert_asset(args: argparse.Namespace) -> None:
    ...
    with open(str(args.out_ct) + ".synthetic_nodule.json", "w", encoding="utf-8") as f:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    a.add_argument("--label", type=int, default=1, help="Label value for the nodule in the source mask")
